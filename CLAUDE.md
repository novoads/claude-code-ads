...
- **First-time setup:** if `.env` is missing, run `./scripts/setup.sh`. If `MASTER_CONTEXT.md` is missing, copy `MASTER_CONTEXT.template.md` to `MASTER_CONTEXT.md`. A setup session's final message is the short template in [AGENTS.md](AGENTS.md) ("First-time setup"): a status line, "What you can ask for now", the product-photos line — and nothing else unless it blocks an ask.

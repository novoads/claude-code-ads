# Claymation storyboard — `gpt-image-2`
...
1. **Always pass the prior approved protagonist still in `referenceAssetIds`** when generating the next beat that includes the protagonist. Beats 1 → 2 → 4 → 6 → 7 → 8 should chain. Beat 3 attaches Beat 1 (or Beat 2) as the protagonist reference; the supporting character is generated fresh from the cast sheet.

   **The cap is 4 references per call**, images only, order preserved and addressable in the prompt as `@Image1`…`@Image4`. An 8-beat chain will want more than that by the later beats, so keep a **rolling window anchored on the hero**: `[hero, N-1, N-2, N-3]`. Never feed a *drifted* still forward — one bad frame in the window teaches every later beat the drift, which is exactly the failure this chain exists to prevent. The cap is also a guardrail: too many style references dilute identity, and the documented fix for that is to cut back to two.

   Upload each still **once** — the `assetId` is durable and reusable without limit, so the hero is uploaded one time and referenced by every beat that needs it.
...
- [ ] **9:16 aspect ratio** — sent explicitly as `aspectRatio: "9:16"`, not left to the `1:1` default
- [ ] **Lower third has clean negative space** on beat 8 for caption overlay

2-retry cap per beat: regenerate a defective still with a corrected prompt without re-confirming, at most twice. **Each retry is billed** — report the extra credits at the end of the run. If the third attempt still loses clay texture or character identity, **try `nano-banana-pro` for that specific beat** (it tends to hold texture better on close-ups and product props). Don't switch the entire ad — beat-by-beat fallback. Re-estimate when you switch: the image schedules differ by more than 3× across the set, so a quote taken for `gpt-image-2` does not describe a Nano Banana run.

## When the user has a brand character sheet already

If they provide an existing claymation-style hero image, skip the cast-sheet protagonist build. **Check `references/` at the repo root first** — that is where product shots, actor stills and style boards live, and it is gitignored, so the files are the user's own. Upload the hero once and pass its `assetId` in `referenceAssetIds` on every protagonist beat. Beat 5 (infographic) doesn't need it.

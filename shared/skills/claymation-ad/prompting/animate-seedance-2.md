...
- **`resolution`:** leave it unset and take the `720p` default. The field **does** exist on `seedance-2.0` — `480p`, `720p`, `1080p`, `4k` (verified live 2026-08-04; an earlier version of this file said it did not exist, which was true of an older deployment). It **multiplies the price**: `480p` is the same as `720p`, `1080p` ≈2.5x, `4k` ≈5x — and an 8–12 beat ad pays that on every beat. Raise it only if the user asks for a high-res finish, and price it with `POST /v1/estimates` first; never quote a credit number from those ratios. **On `seedance-2.0-mini` do not send the key at all** — it renders 720p only. Default output measured 720x1280 at `9:16` (2026-08-02). Do not carry that number to other models.

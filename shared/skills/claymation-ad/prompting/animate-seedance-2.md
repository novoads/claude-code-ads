...
- **Audio:** there is no `audioEnabled` field. Seedance renders audio and lip-sync in the same call and the prompt is the only control — see the "No in-prompt narrator" rule below. Ambient SFX language is fine; a `Narrator:` line is not.
- **Prompt length:** `Subject + Action + Camera + Style + Constraints`, with length following the **shot count** rather than a fixed word band — roughly 90–130 words for a one-shot prompt, roughly 250–330 for a 3–4 beat one. Each of the 8 beats is one clip and one shot, but the beat prompts below run 180–270 words because each re-describes the approved still in full and carries its own claymation constraints block; across eight beats that redundancy is what holds the character together, not padding. **The one hard limit is 4,000 characters** — the longest prompt in this file is under 1,800. **No `Narrator:` line.**

...
- **`seedance-2.0` does have a `resolution` field** — `480p`, `720p`, `1080p`, `4k`, default `720p` (verified live 2026-08-04; the older note here denying the field described a previous deployment). **It is still not a way to trade quality for cost: `480p` costs exactly the same as `720p`.** The tiers only go up — `1080p` ≈2.5x the base, `4k` ≈5x — and on a multi-beat ad that multiplier lands on every beat. Anything above `720p` is a spend decision: price it with `POST /v1/estimates` and never quote a credit number from those ratios.
- **The cheap-draft lever is the model**: `seedance-2.0-mini` is half the price of `seedance-2.0` on the same grid (720p only — never send it a `resolution` key). Prototype the beat timing on mini, finalize on `seedance-2.0`.

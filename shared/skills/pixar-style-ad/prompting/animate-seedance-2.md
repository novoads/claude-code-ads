...
- **Prompt length:** `Subject + Action + Camera + Style + Constraints`, with length following the **shot count** rather than a fixed word band — roughly 90–130 words for a one-shot prompt, roughly 250–330 for a 3–4 beat one. Every beat here is one clip and one shot, but the patterns below run 160–230 words because each re-describes the approved start frame in full and carries its own constraints block; that is the identity lock this pipeline exists for, not padding. **The one hard limit is 4,000 characters**, enforced on the estimate as well as the paid call — the longest prompt in this file is under 1,700.

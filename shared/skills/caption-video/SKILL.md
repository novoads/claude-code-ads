...
name: caption-video
description: >-
  Burn timed, styled captions into a finished video without re-rendering the
  source. Transcribes the audio with Whisper, groups words into reading phrases,
  renders the captions in a real browser via HyperFrames, and composites them
  over the source with ffmpeg using a real alpha channel. Use when the user has
  an MP4 with narration, dialogue or voiceover and asks to add captions,
  subtitles, burned-in text, TikTok-style captions or word-by-word text to it.
  Entirely out of band — no API calls, no credits, works on any video file
  whoever generated it. Not for generating video (use the video skills) and not
  for a separate .srt/.vtt sidecar file.
---

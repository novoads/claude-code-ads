...
- The `novoads-api` skill — the upstream pipeline that produces the source videos
- The `claymation-ad` and `pixar-style-ad` guides — produce the kind of source video this
  skill captions, and carry the "no dead space" trimming rule referenced at the top

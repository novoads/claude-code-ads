...
| **Pixar-style ad, multi-beat** | The storyboard flow: cast sheet → per-beat `gpt-image-2` stills, each chaining the previous one's `assetId` to hold the character → `seedance-2.0` per beat → `POST /v1/voiceovers` into the gaps → music bed → captions → local ffmpeg assembly. Gated: the board is reviewed before the clips are paid for | [novoads-pixar-storyboard-ad](skills/novoads-pixar-storyboard-ad/SKILL.md) |
